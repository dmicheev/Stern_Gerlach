...
(`./scripts/build-wasm.sh`, на этой машине — `./scripts/build-wasm-macos-arm64.sh`)
и коммита бинарника. Иначе в браузере остаётся
...
```bash
./scripts/build-wasm-macos-arm64.sh  # оболочка над wasm-pack со всеми env (эта машина)
./scripts/build-wasm.sh              # переносимый вариант (Linux/целый rustup)
```
